    ...
    public connect(): void {
        ...
        this.peerConnection.addEventListener('negotiationneeded', async () => {
            try {
                if (!!this.peerConnection) {
                    await this.peerConnection && this.peerConnection.setLocalDescription(await this.peerConnection.createOffer());
                    socket.emit('controller-description', this.peerConnection.localDescription);
                }
            } catch (error) {
        ...
        socket.on('host-description', async (description: RTCSessionDescription) => {
            if (description.type === 'offer') {
                ...
                this.emitEvent(EventType.Error, `Unsupported UDP type: ${description.type}`);
            }
        });
